...
import { ensureSeedData, saveActivities, saveBudgets, saveChecklistItems, saveContractors, saveEvents, saveQuestions, saveSettings, saveTemplates, type EventFlowData } from '../services/storage.ts'
...
export function EventFlowProvider({ children }: { children: ReactNode }) {
  const [data, setData] = useState(() => ensureSeedData())
  ...
  const value = useMemo<EventFlowContextValue>(() => ({
    ...
    addEvent: (event) => setData((current) => {
      ...
      const activities = [makeActivity({ eventId: event.id, type: 'event', title: 'Создано мероприятие', description: event.title }), ...current.activities]
    ...
    updateEvent: (event) => setData((current) => {
      ...
      const activities = [makeActivity({ eventId: event.id, type: 'event', title: 'Обновлены данные мероприятия', description: event.title }), ...current.activities]
      saveEvents(events); saveActivities(activities)
      return { ...current, events, activities }
    }),
    updateChecklistItem: (item) => setData((current) => {
      const updated = { ...item, updatedAt: new Date().toISOString(), completedAt: item.status === 'done' ? item.completedAt ?? new Date().toISOString() : null }
      ...
      const events = current.events.map((event) => event.id === item.eventId ? { ...event, progress, updatedAt: new Date().toISOString() } : event)
      const activities = [makeActivity({ eventId: item.eventId, type: 'checklist', title: item.status === 'done' ? 'Выполнен пункт чек-листа' : 'Изменён пункт чек-листа', description: item.title }), ...current.activities]
    ...
    addChecklistItem: (item) => setData((current) => {
      ...
      const progress = calculateChecklistProgress(eventItems)
      const events = current.events.map((event) => event.id === item.eventId ? { ...event, progress, updatedAt: new Date().toISOString() } : event)
      const activities = [makeActivity({ eventId: item.eventId, type: 'checklist', entityId: item.id, title: 'Создана задача', description: item.title }), ...current.activities]
      saveChecklistItems(checklistItems); saveEvents(events); saveActivities(activities)
      return { ...current, checklistItems, events, activities }
    }),
    addQuestion: (question) => setData((current) => {
      const questions = [question, ...current.questions]
      const activities = [makeActivity({ eventId: question.eventId, type: 'question', title: 'Создан открытый вопрос', description: question.title }), ...current.activities]
    ...
    updateQuestion: (question) => setData((current) => {
      ...
      const activities = [makeActivity({ eventId: question.eventId, type: 'question', title: question.status === 'resolved' ? 'Закрыт вопрос' : 'Изменён открытый вопрос', description: question.title }), ...current.activities]
      saveQuestions(questions); saveActivities(activities)
      return { ...current, questions, activities }
    }),
    saveBudget: (budget) => setData((current) => {
      ...
      const activities = [makeActivity({ eventId: budget.eventId, type: 'budget', title: 'Изменён бюджет', description: 'Обновлён локальный расчёт' }), ...current.activities]
    ...
    addExpense: (eventId, amount, description) => setData((current) => {
      ...
      const activities = [makeActivity({ eventId, type: 'budget', entityId: eventId, amount, title: 'Добавлен расход', description }), ...current.activities]
      saveBudgets(budgets); saveActivities(activities)
      return { ...current, budgets, activities }
    }),
    saveContractor: (contractor) => setData((current) => {
      ...
      const activities = [makeActivity({ eventId: '', type: 'contractor', title: exists ? 'Обновлён контрагент' : 'Добавлен контрагент', description: contractor.name }), ...current.activities]
